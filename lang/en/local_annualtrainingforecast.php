<?php
// This file is part of Moodle - http://moodle.org/
//
// Moodle is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// Moodle is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with Moodle.  If not, see <http://www.gnu.org/licenses/>.

/**
 * English language strings
 *
 * @package    local_annualtrainingforecast
 * @copyright  2025 Mattan Dor (CentricApp)
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

$string['pluginname'] = 'Annual Training Forecast';
$string['managecourses'] = 'Manage Courses';
$string['viewreports'] = 'View Reports';
$string['addcourse'] = 'Add Course';
$string['editcourse'] = 'Edit Course';
$string['deletecourse'] = 'Delete Course';
$string['coursename'] = 'Course Name';
$string['description'] = 'Description';
$string['duration'] = 'Duration (days)';
$string['startdate'] = 'Start Date';
$string['enddate'] = 'End Date';
$string['status'] = 'Status';
$string['status_upcoming'] = 'Upcoming';
$string['status_inprogress'] = 'In Progress';
$string['status_completed'] = 'Completed';
$string['status_cancelled'] = 'Cancelled';
$string['completed'] = 'Completed';
$string['notcompleted'] = 'Not Completed';
$string['parentcourse'] = 'Parent Course';
$string['iterations'] = 'Iterations';
$string['additeration'] = 'Add Iteration';
$string['editeration'] = 'Edit Iteration';
$string['deleteiteration'] = 'Delete Iteration';
$string['noiterations'] = 'No iterations found';
$string['confirmdelete'] = 'Are you sure you want to delete this item?';
$string['yearview'] = 'Year View';
$string['halfyearview'] = 'Half Year View';
$string['quarterlyview'] = 'Quarterly View';
$string['updatefailed'] = 'Update Failed';
$string['enddatebeforestartdate'] = 'End date cannot be before start date';
$string['exportexcel'] = 'Export to Excel';
$string['exportpdf'] = 'Export to PDF';
$string['statussummary'] = 'Status Summary';
$string['count'] = 'Count';
$string['completionsummary'] = 'Completion Summary';
$string['monthlydistribution'] = 'Monthly Distribution';
$string['month'] = 'Month';
$string['category'] = 'Course Category';
$string['category_help'] = 'Select the category where the course will be created. You can only select categories where you have permission to create courses.';
$string['existingcourse'] = 'Select Existing Course';
$string['newcourse'] = 'Create New Course';
$string['courseselectiontype'] = 'Course Selection';
$string['courseselectiontype_help'] = 'Choose whether to select an existing course or create a new one.';

$string['copymaterials'] = 'Copy materials from parent course';
$string['copymaterials_help'] = 'If checked, all course materials (activities, resources, etc.) will be copied from the parent course to this new iteration';
$string['linkexisting'] = 'Link to existing course';
$string['linkexisting_help'] = 'If checked, this iteration will be linked to an existing course in the system instead of creating a new one';

$string['ganttview'] = 'Gantt View';
$string['reports'] = 'Reports';
$string['courseupdated'] = 'Course updated successfully';
$string['courseadded'] = 'Course added successfully';
$string['instanceupdated'] = 'Iteration updated successfully';
$string['instanceadded'] = 'Iteration added successfully';
$string['cannotdeletecourse'] = 'Cannot delete course';
$string['deletecoursewarning'] = 'This will also delete all iterations associated with this course.';
$string['coursedeleted'] = 'Course deleted successfully';
$string['deleteiterationwarning'] = 'This action cannot be undone.';
$string['instancedeleted'] = 'Iteration deleted successfully';
$string['addparentcourse'] = 'Add Parent Course';
$string['nocourses'] = 'No courses found';
$string['addcourseinstance'] = 'Add Course Instance';
$string['courseduration'] = 'Course Duration';
$string['createnewcourse'] = 'Create New Course';
$string['selectexistingcourse'] = 'Select Existing Course';
$string['createtheoreticalcourse'] = 'Theoretical course only (no Moodle course)';
$string['coursesource'] = 'Course Source';
$string['coursesource_help'] = 'Choose whether to create a new Moodle course, link an existing one, or add a theoretical course for planning only without creating a physical Moodle course.';
$string['theoreticalonly'] = 'Theoretical iteration only';
$string['theoreticalonly_help'] = 'If checked, this iteration will be added for planning only without creating or linking a Moodle course.';
$string['theoreticalbadge'] = 'Theoretical';
$string['parenthasnomoodlecourse'] = 'The parent course has no associated Moodle course. Create a theoretical iteration, or link an existing course.';
$string['newcoursedetails'] = 'New Course Details';
$string['coursedescription'] = 'Course Description';
$string['existingcoursedetails'] = 'Existing Course Details';
$string['selectcourse'] = 'Select Course';
$string['viewcourse'] = 'View Course';
$string['previousyear'] = 'Previous Year';
$string['nextyear'] = 'Next Year';
$string['currentyear'] = 'Current Year';
$string['moreactivities'] = '+{$a} more';
$string['calendarlegend'] = 'Status colours';
$string['generalevent'] = 'General event';
$string['addgeneralevent'] = 'Add general event';
$string['eventtitle'] = 'Event title';
$string['eventtitlerequired'] = 'Please enter an event title.';
$string['eventdescription'] = 'Description';
$string['eventstartdate'] = 'Start date';
$string['eventenddate'] = 'End date';
$string['deletegeneralevent'] = 'Delete event';
$string['confirmeventdelete'] = 'Delete this event? This cannot be undone.';
$string['dayclickhint'] = 'Tip: click a day to add a general event (managers).';

$string['pageheading'] = 'Annual Training Forecast';
$string['today'] = 'Today';
$string['previous'] = 'Previous';
$string['next'] = 'Next';
$string['view_month'] = 'Month';
$string['view_quarter'] = 'Quarter';
$string['view_halfyear'] = 'Half Year';
$string['view_year'] = 'Year';
$string['searchplaceholder'] = 'Search trainings...';
$string['filtercategory'] = 'Category';
$string['filterstatus'] = 'Status';
$string['filtermanager'] = 'Manager';
$string['allcategories'] = 'All categories';
$string['allstatuses'] = 'All statuses';
$string['allmanagers'] = 'All managers';
$string['status_general'] = 'General Event';
$string['legend'] = 'Status Legend';
$string['legend_hint'] = 'Hold Ctrl + scroll to zoom';
$string['trainingcolumn'] = 'Training / Course';
$string['loading'] = 'Loading…';
$string['close'] = 'Close';
$string['addtraining'] = '+ Add Training';
$string['addevent'] = '+ Add Event';
$string['save'] = 'Save';
$string['cancel'] = 'Cancel';
$string['delete'] = 'Delete';
$string['tip_category'] = 'Category';
$string['tip_manager'] = 'Manager';
$string['tip_start'] = 'Start';
$string['tip_end'] = 'End';
$string['tip_duration'] = 'Duration';
$string['tip_status'] = 'Status';
$string['tip_date'] = 'Date';
$string['tip_type'] = 'Type';
$string['tip_note'] = 'Note';
$string['duration_day'] = 'day';
$string['duration_days'] = 'days';
$string['wd_su'] = 'Su';
$string['wd_mo'] = 'Mo';
$string['wd_tu'] = 'Tu';
$string['wd_we'] = 'We';
$string['wd_th'] = 'Th';
$string['wd_fr'] = 'Fr';
$string['wd_sa'] = 'Sa';
$string['timelinearea'] = 'Gantt timeline area';
$string['traininglist'] = 'Training list';
$string['err_startdate_required'] = 'Start date is required.';
$string['err_enddate_required'] = 'End date is required.';
$string['err_eventdate_required'] = 'Event date is required.';
$string['err_generic'] = 'An error occurred. Please try again.';
$string['no_trainings_found'] = 'No trainings found for this period.';
$string['navigate_hint'] = 'Navigate with ❮ ❯, or click + Add Training.';
$string['saving'] = 'Saving…';
?>
